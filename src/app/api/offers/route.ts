...
export async function GET(req: NextRequest) {
  ...
  if (dateFrom || dateTo) {
    where.offerDate = {};
    if (dateFrom) where.offerDate.gte = parseInt(dateFrom);
    if (dateTo) where.offerDate.lte = parseInt(dateTo);
  }
...
export async function POST(req: NextRequest) {
  ...
  const { title, product, duration, openStatus, value, companyId, documentUrl, offerDate } = await req.json();
  ...
  const offer = await prisma.offer.create({
    data: {
      ...
      offerDate: offerDate ? parseInt(offerDate) : null,
