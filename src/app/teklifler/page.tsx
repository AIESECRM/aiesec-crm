...
import { DollarSign, Filter, Plus, User, Building2, Calendar, Percent, X, RotateCcw, Save, FileText } from 'lucide-react';
import Link from 'next/link';
import { useAuth } from '@/contexts/AuthContext';
import { getCompanies } from '@/actions/companies';
import { getAllProposals, createProposal, updateProposal } from '@/actions/proposals';
import { DealStage, Proposal, Company } from '@/types';
...
export default function DealsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (user?.id && user?.id !== 'loading') {
        const [fetchedCompanies, fetchedProposals] = await Promise.all([
          getCompanies(user.id),
          getAllProposals(user.id)
        ]);
  ...
  const uniqueOwners = React.useMemo(() => {
    return Array.from(new Set(proposals.map(p => p.ownerName)));
  }, [proposals]);
  ...
  const handleAddDeal = async (e: React.FormEvent) => {
    ...
    if (result) {
      ...
      alert(`Teklif eklendi: ${result.title}`);
  ...
  const handleDrop = useCallback(async (e: React.DragEvent, targetStage: DealStage) => {
    e.preventDefault();
    ...
    if (proposalId) {
      ...
      const updated = await updateProposal(proposalId, { stage: targetStage }, user.id);
      if (!updated) {
        if (user?.id) {
          const fetched = await getAllProposals(user.id);
  ...
  const handleTouchEnd = useCallback(async (e: React.TouchEvent) => {
    ...
    let targetStageId: DealStage | null = null;
    columns.forEach(column => {
      const rect = column.getBoundingClientRect();
      if (touch.clientX >= rect.left && touch.clientX <= rect.right &&
        touch.clientY >= rect.top && touch.clientY <= rect.bottom) {
        targetStageId = column.getAttribute('data-stage') as DealStage;
      }
    });

    if (targetStageId) {
      const proposalId = touchStartRef.current.proposalId;
      setProposals(prev => prev.map(p =>
        p.id === proposalId ? { ...p, stage: targetStageId! } : p
      ));

      const updated = await updateProposal(proposalId, { stage: targetStageId! }, user.id);
      if (!updated) {
        if (user?.id) {
          const fetched = await getAllProposals(user.id);
          setProposals(fetched);
                    ...
                    >
                      ...
                      <div className="deal-card__footer">
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <Link href={`/sozlesme/${proposal.id}`} target="_blank" title="Sözleşme Yazdır">
                            <FileText size={14} style={{ color: 'var(--text-light)', cursor: 'pointer' }} />
                          </Link>
